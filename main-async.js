...
jQuery(document).ready(function ($) {
    ...
    $('body').on('click', '.nasa-popup-exit-intent-close ', function (e) {
        var ns_pexit_intent = $('.nasa-popup-exit-intent-wrap');

        if ($(ns_pexit_intent).length) {

            $(ns_pexit_intent).addClass('ns-left-to-right');
            $('.black-window').fadeOut(500).removeClass('desk-window');
            $('body').removeClass('ovhd');
            $.cookie('nasa_popup_exit_intent_ck', 'false', {expires: _cookie_live, path: '/'});
            setTimeout(function () {
                $(ns_pexit_intent).removeClass('nasa-active ns-left-to-right').hide();
            }, 350);
        }
    });
